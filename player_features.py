...
GOALSCORERS_CACHE_PATH = SCRIPT_DIR / "data_cache" / "goalscorers.csv"
...
def load_goalscorers(path: Path | None = None) -> pd.DataFrame:
    ...
    if not path.exists() and path == GOALSCORERS_PATH:
        GOALSCORERS_CACHE_PATH.parent.mkdir(parents=True, exist_ok=True)
        if not GOALSCORERS_CACHE_PATH.exists():
            resp = requests.get(GOALSCORERS_URL, timeout=120)
            resp.raise_for_status()
            GOALSCORERS_CACHE_PATH.write_bytes(resp.content)
            print(f"goalscorers downloaded -> {GOALSCORERS_CACHE_PATH}")
        path = GOALSCORERS_CACHE_PATH
    g = pd.read_csv(path)
