...
def _default_goalscorers_path() -> Path:
    if GOALSCORERS_PATH.exists():
        return GOALSCORERS_PATH
    if not CACHE_GOALSCORERS_PATH.exists():
        ...
        print(f"goal scorers updated via download -> {CACHE_GOALSCORERS_PATH}")
    return CACHE_GOALSCORERS_PATH


def load_goalscorers(path: Path | None = None) -> pd.DataFrame:
    path = path or _default_goalscorers_path()
    g = pd.read_csv(path)
